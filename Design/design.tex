...
\begin{itemize}
    ...
    \item Clicking the Remove Entry Button will prompt a seperate interface to open, which contains a view of the database. If an entry was selected beforehand, then upon clicking Remove Entry, The user will be prompted for confirmation, then asked to enter their password. Entries can be searched for and then can be selected and deleted, and the user will be prompted for confirmation
    \item Clicking the Edit Entry button will prompt a seperate interface to open, and will contain a view of the database. An entry can be searched for using the search, selected, and once the user clicks "Edit", the window will be replaced by the window for Adding Entries, but the data will be in their respective entry fields already. If an entry was selected before hand, the window for Adding entries will open first upon clicking Edit Entry, and the data will be in the fields already, ready for editing. Then, the data can be edited and saved, and the user will be prompted for confirmation then asked to enter their password.
\end{itemize}
...
\begin{figure}[H]
    \caption{Publishing Invoice} \label{PubInvoice.pdf}
    \includegraphics[width=\textwidth]{./Design/UserInterfaceDesign/PubInvoice.pdf}
\end{figure}
